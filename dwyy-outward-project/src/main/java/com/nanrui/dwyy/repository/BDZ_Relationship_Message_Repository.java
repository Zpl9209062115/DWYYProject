...
public interface BDZ_Relationship_Message_Repository extends JpaRepository<BDZ_Relationship_Message,String> {
    ...
    @Query(value = "SELECT pms_id_start,dtdj_start,name_start,pms_id_end,dydj_end,name_end,line_id,line_name,yyxl,city,city_code FROM substation_particulars_gs where city_code = ?1 and dydj_end like ?2")
    public List<Object[]> getBDZ_Message(String cityCode,String condition);

    @Query(value = "SELECT pms_id_start,dtdj_start,name_start,pms_id_end,dydj_end,name_end,line_id,line_name,yyxl,city,city_code FROM substation_particulars_gs where dydj_end like ?1")
    public List<Object[]> getBDZ_Message_condition(String condition);

    @Query(value = "SELECT pms_id_start,dtdj_start,name_start,pms_id_end,dydj_end,name_end,line_id,line_name,yyxl,city,city_code FROM substation_particulars_gs where city_code = ?1")
    public List<Object[]> getBDZ_Message_city(String cityCode);

    @Query(value = "SELECT pms_id_start,dtdj_start,name_start,pms_id_end,dydj_end,name_end,line_id,line_name,yyxl,city,city_code FROM substation_particulars_gs")
    ...
    @Query(value = "SELECT city,city_code,dtdj_start,dydj_end,name_end,name_start,pms_id_end,pms_id_start,x_coord_end,x_coord_start,y_coord_end,y_coord_start,line_id,line_name,yyxl FROM substation_particulars_bdz_message WHERE x_coord_end <> 0 AND x_coord_end <> 0 AND y_coord_end <> 0 AND y_coord_start <> 0 AND line_name <> 0 AND yyxl <> 0")
    public List<Object[]> getBDZ_MessageForHtml();

    @Query(value = "SELECT city,city_code,dtdj_start,dydj_end,name_end,name_start,pms_id_end,pms_id_start,x_coord_end,x_coord_start,y_coord_end,y_coord_start,line_id,line_name,yyxl FROM substation_particulars_bdz_message WHERE x_coord_end <> 0 AND x_coord_end <> 0 AND y_coord_end <> 0 AND y_coord_start <> 0 AND line_name <> 0 AND yyxl <> 0 AND dydj_end like ?1")
